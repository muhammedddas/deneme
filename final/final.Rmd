...
tibble(expand.grid(years=c( '2011', '2010' )))
...
cy %>%
  select("x15age_risk_of_poverty","numberof_poors_thousand_person", "illiterate4" , "literatewith_no_degree_5", "lessthan_high_school","highschool_or_equivalent_7", "highereducation_8", "illiterate10" , "literatewith_no_degree_11" , "lessthan_high_school_2" , "highschool_or_equivalent_13","highereducation_14") %>% 
...
```{r plot, echo=FALSE, fig.align='center', fig.cap='Grafik 1'}
cy %>% 
ggplot(aes(x =highereducation_14  , y = illiterate10 )) +
  geom_point(na.rm = TRUE) +
  geom_smooth() +
  scale_x_continuous("highereducation_14") + 
  scale_y_continuous("illiterate10")
...
ggplot(aes(x = highereducation_8  , y = illiterate4 )) +
  geom_point(na.rm = TRUE) +
  geom_smooth() +
  scale_x_continuous("highereducation_8") + 
  scale_y_continuous("illiterate4")

ggplot(cy) +
  aes(x =highereducation_8  ) +
  geom_histogram( na.rm = TRUE)
...
shapiro.test(cy$illiterate10)

shapiro.test(cy$highereducation_14)
...
tibble(x = rnorm(100)) %>% 
  ...
  stat_qq() +
  stat_qq_line() +
  labs(title = "Normal Dağılım için Q-Q Grafiği")
```
...
ggplot(cy, aes(x=illiterate4, y=highereducation_8)) +
  geom_line()
